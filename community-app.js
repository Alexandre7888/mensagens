class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) return <div className="p-8 text-center"><h1 className="text-red-500">Erro</h1></div>;
...
function CommunityApp() {
  ...
  const [initialCommId, setInitialCommId] = React.useState(null);

  React.useEffect(() => {
    const init = async () => {
      try {
        const params = new URLSearchParams(window.location.search);
        const cid = params.get("commId");
        if (cid) setInitialCommId(cid);
  ...
  return (
    <>
      <CommunityList user={userData} initialCommId={initialCommId} />
...
root.render(<ErrorBoundary><CommunityApp /></ErrorBoundary>);