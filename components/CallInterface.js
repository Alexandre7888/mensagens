function CallInterface({ user, callId }) {
    ...
    React.useEffect(() => {
        ...
        newPeer.on('call', (call) => {
            ...
            const reqVideo = cData?.isVideo === true;
            navigator.mediaDevices.getUserMedia({ video: reqVideo, audio: true })
            ...
            .then(stream => {
                ...
                console.error("Media error incoming", err);
                setCallState('ended');
            });
        });
    ...
    // Enviar notificação de chamada a cada 2 segundos enquanto estiver chamando
    React.useEffect(() => {
        let interval = null;
        if (callData && callData.callerId === user.id && (callState === 'connecting' || callState === 'waiting')) {
            const receiverId = callData.receiverId;
            const callUrl = `${window.location.origin}/call.html?callId=${callId}`;
            
            // Envia imediatamente
            api.sendCallNotification([receiverId], callUrl);
            
            // Continua enviando a cada 2 segundos
    ...
    }, [callData, callState, user.id, callId]);
